...
class PerceiverIOSequenceEncoder(nn.Module):
    """
    Input:  x (B, N, in_dim)  -- e.g. (B, 20000, 1536)
    Output: y (B, N, dim)     -- e.g. (B, 20000, 256)

    Per-layer structure:
      1) latents <- tokens  (cross-attn)
      2) latents <-> latents (self-attn + FFN)
      3) tokens  <- latents  (cross-attn)  # write back to per-token representation
      4) tokens  FFN
    """
    def __init__(
        self,
        ...
        track_attn: bool = True,   # set True to store the last-layer attention for visualization
    ):
        super().__init__()
        self.dim = dim
        ...
        # optional: expose the last tokens<-latents attn weights (B, heads, N, L)
    ...
    def forward(self, x, key_padding_mask=None):  # x: (B, N, in_dim)
        ...
        for blk in self.blocks:
            ...
            # 3) tokens <- latents (cross-attn)   (keep N unchanged)
            qt = blk.ln_tok_x2(x)
