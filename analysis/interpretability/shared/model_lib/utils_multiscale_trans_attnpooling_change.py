...
MODEL_LIB_DIR = os.path.dirname(os.path.abspath(__file__))
MULTISCALE_DIR = os.path.join(MODEL_LIB_DIR, "MultiScale")
if MULTISCALE_DIR not in sys.path:
    sys.path.insert(0, MULTISCALE_DIR)
if MODEL_LIB_DIR not in sys.path:
...
class utils_multiScale_model_trans(nn.Module):
    """Interpretability-compatible XBM adapter with scale-fusion hooks."""
